...
export default function AdminSidebar({
  ...
  const navItems = [
    { label: "Bảng điều khiển", path: "/center-manager/dashboard", icon: LayoutDashboard },
    { label: "Nhân sự", path: "/center-manager/users", icon: Users },
      ...
      <div>
        ...
        <div className={`flex items-center ${isCollapsed ? "justify-center flex-col gap-2" : "justify-between"} pb-4 border-b border-slate-200/60 dark:border-slate-800/60 min-h-[56px]`}>
          <Link to="/center-manager/dashboard" className="flex items-center gap-3 group shrink-0">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600 text-white flex items-center justify-center shadow-[0_4px_12px_rgb(99,102,241,0.3)] group-hover:scale-105 group-hover:shadow-[0_8px_20px_rgb(99,102,241,0.4)] transition-all duration-300 shrink-0">
              <BrainCircuit className="w-6 h-6" />
            </div>
            {!isCollapsed && (
              <div className="transition-opacity duration-200">
                <span className="font-extrabold text-lg text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-violet-600 dark:from-indigo-400 dark:to-violet-400 tracking-tight block leading-tight truncate">
                  TechEdu
                </span>
              </div>
            )}
                ...
                className={
                  isCollapsed
                    ? `w-12 h-12 rounded-xl flex items-center justify-center mx-auto transition-all duration-200 ${
                        active
                          ? "bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 shadow-[inset_0_1px_3px_rgb(0,0,0,0.05)] border border-indigo-100 dark:border-indigo-500/30 scale-[0.98]"
                    ...
                    : `flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm font-semibold transition-all duration-200 group ${
                        active
                          ? "bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 shadow-[inset_0_1px_3px_rgb(0,0,0,0.05)] border border-indigo-100 dark:border-indigo-500/30 scale-[0.99]"
                          : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-100"
                      }`
                }
              >
                <Icon className={`w-5 h-5 shrink-0 transition-colors ${active ? "text-indigo-600 dark:text-indigo-400" : "text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 dark:group-hover:text-indigo-400"}`} />
        ...
        <div className={`flex items-center ${isCollapsed ? "justify-center p-1.5" : "justify-between gap-2 p-3"} rounded-2xl
          ...
          {!isCollapsed && (
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-slate-800 dark:text-slate-100 truncate">{managerName}</p>
              {user?.email && (
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{user.email}</p>
              )}
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold
                bg-violet-100/60 dark:bg-violet-900/40
